        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setFilterType('all')} className={`px-4 py-2 rounded-xl text-sm font-medium transition ${filterType === 'all' ? 'bg-[#00d4ff] text-black' : 'bg-[#0f1422] text-[#b0bedb] border border-[#2a3440] hover:border-[#00d4ff]'}`}>
                All
              </button>
              <button onClick={() => setFilterType('html')} className={`px-4 py-2 rounded-xl text-sm font-medium transition ${filterType === 'html' ? 'bg-[#00d4ff] text-black' : 'bg-[#0f1422] text-[#b0bedb] border border-[#2a3440] hover:border-[#00d4ff]'}`}>
                📘 HTML
              </button>
              <button onClick={() => setFilterType('pdf')} className={`px-4 py-2 rounded-xl text-sm font-medium transition ${filterType === 'pdf' ? 'bg-[#00d4ff] text-black' : 'bg-[#0f1422] text-[#b0bedb] border border-[#2a3440] hover:border-[#00d4ff]'}`}>
                📕 PDF
              </button>
            </div>
          </div>
        ...
        {filteredEbooks.length === 0 ? (
          <div className="text-center py-16 bg-[#0f1422] rounded-2xl">
            ...
            {(searchQuery || filterType !== 'all') && (
              <button onClick={() => { setSearchQuery(''); setFilterType('all'); }} className="mt-4 text-[#00d4ff] hover:underline text-sm">
                Clear all filters
              </button>
            )}
          </div>
                      ...
                      {isInlineSvg(ebook.cover_image_url) ? (
                        ...
                        <img 
                          ...
                          onError={(e) => {
                            ...
                            e.target.parentElement.innerHTML = '<div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#0f1422] to-[#0a0e1a]"><span class="text-5xl">📘</span></div>';
                          }}
                        />
                      )}
                    </div>
