...
export default function EbooksManager() {
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (!title || !file) { setError('Title and PDF required'); return; }
    setLoading(true);
    setError('');
  ...
  const handleDelete = async (id) => {
    ...
    try { await deleteEbook(id); loadEbooks(); } catch (err) { alert('Delete failed'); }
  };
  ...
  return (
    <div className="p-6">
      ...
      <form onSubmit={handleUpload} className="bg-[#0f1422] p-5 rounded-xl mb-8 space-y-4">
        ...
        <input type="text" placeholder="Ebook Title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-[#131724] border border-[#2a3440] rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#00d4ff]" required />
        ...
        <button type="submit" disabled={loading} className="bg-[#00d4ff] text-black px-5 py-2 rounded-full hover:bg-[#00b8e6] disabled:opacity-50">{loading ? 'Uploading...' : 'Upload Ebook'}</button>
      </form>
      <h2 className="text-xl font-semibold text-white mb-4">Existing Ebooks</h2>
      {ebooks.length === 0 ? <p className="text-[#b0bedb]">No ebooks yet.</p> : (
        <div className="space-y-3">
          {ebooks.map(ebook => (
            <div key={ebook.id} className="bg-[#0f1422] p-4 rounded-xl flex justify-between items-center flex-wrap gap-2">
              <div><h3 className="text-lg font-semibold text-white">{ebook.title}</h3><p className="text-sm text-[#b0bedb]">{ebook.description?.slice(0, 100)}</p></div>
