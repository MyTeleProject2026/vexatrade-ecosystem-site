  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <input type="text" placeholder="Post Title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full bg-[#0f1422] border border-[#2a3440] rounded-lg px-4 py-2 text-white focus:outline-none focus:border-[#00d4ff]" required />
        ...
        <div className="flex gap-3">
          <button type="submit" disabled={loading} className="bg-[#00d4ff] text-black px-6 py-2 rounded-full hover:bg-[#00b8e6] disabled:opacity-50">{loading ? 'Saving...' : 'Save Post'}</button>
          <button type="button" onClick={() => navigate('/posts')} className="border border-[#2a3440] px-6 py-2 rounded-full hover:bg-white/5">Cancel</button>
        </div>
      </form>
    </div>
  );
